	...
	execute(message, args) {
		...
		message.guild.createChannel(`ticket-${id}`, {
			...
			if (config.tagHereOnly) {
				...
				await c.send(`[!]  <@&${config.supportRole}>  [!]\n\n**${message.author} wants to buy a FiveM-Key.**\n`);
			};
			...
			const created = new Discord.RichEmbed()
				...
				.setDescription(`Your ticket [ ${c} ] has been successfully created.\nPlease read the information and follow the instructions.`)
				.setTimestamp();
			const welcome = new Discord.RichEmbed()
				.setColor(config.colour)
				.setDescription(`**E-Books Fivem **\n\n${config.ticketText}`) 
				//				.setDescription(`**E-Books Fivem: ** \`${topic}\`\n\n${config.ticketText}`) 
			...
			if (config.useEmbeds) {
				...
				message.channel.send(`Your ticket [ ${c} ] has been successfully created.\nPlease read the information and follow the instructions.`)
				let w = await c.send(`**E-Books Fivem ** \`${topic}\`\n\n${config.ticketText}`)
				await w.pin()
				// c.fetchMessage(c.lastMessageID).delete()
			...
			if (config.useEmbeds) {
				const embed = new Discord.RichEmbed()
					.setAuthor(`${client.user.username} | STAFF Log`, client.user.avatarURL)
					.setTitle("NEW Ticket")
					.setColor(config.colour)
					.setDescription(`\`${topic}\``)
					.addField("Username", message.author, true)
					.addField("Channel", c, true)
					.setFooter(`E-Books Fivem`)
					.setTimestamp();
